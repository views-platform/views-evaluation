...
- **Config Validation at Construction**: Guarantees that a structurally invalid config fails at `__init__`, not at `evaluate()`. Rejects legacy keys removed in 0.4.0 (an enumerated set, matched by exact name), keys that closely resemble a real evaluation key (a suspected typo), a missing, empty, or non-list `steps`, non-positive step positions, absence of any target list, a declared task with no metrics, and metric names that are unknown or invalid for the cell their key declares. Nothing is defaulted or repaired (ADR-015).
...
- `steps` missing, empty, not a `list`, or containing a non-integer or non-positive entry.
...
- Relying on a misspelled or legacy config key being ignored — a legacy key, and a key that closely resembles a real evaluation key, are both rejected at construction. (An unrecognised key that resembles *nothing* is still ignored, deliberately — see §3 "Tolerates Foreign Config Keys" and register C-33. This bullet claimed "every key is now validated" until 2026-08-02, contradicting §3 of this same contract.)
...
- Config validation added to `__init__` (2026-08-02, C-02): structural config errors now surface at construction rather than at evaluation. The valid key set is derived from `EvaluationConfig.__annotations__`, giving that previously type-only module a runtime authority. Tests: every `TestNativeEvaluatorRed::test_*_rejected_at_init` case. (Cited by naming convention, not by count — two documents previously carried two different counts, neither matching the code. See register C-34.)
...
- Non-list `steps` rejected at construction (2026-08-02, PR #45 review): a scalar `steps` is truthy, so it passed the non-empty check and then raised a bare `TypeError` from the per-element comprehension — a crash naming neither the key nor the expectation. Now a `ValueError`. Restricted to `list` (not `list`-or-`tuple`) to match `EvaluationConfig`'s declared `List[int]`, which is this schema's single authority. Test: `test_scalar_steps_rejected_at_init`.
