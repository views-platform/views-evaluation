...
- Accepted sentinels upstream (`MCR`'s `inf`/`nan`, `Pearson`'s `nan`, `AP`'s `nan` on a group with no positive truth — ADR-015 R1/R2/R9) legitimately reach here, in any cell. The `mean` row excludes the `nan`s (`nanmean`) but not `MCR`'s `inf`, which is a statement rather than a missing answer: one `inf` group makes the mean row `inf`. A frame in which every value is `nan` is emitted with a WARNING logged (R6, amended 2026-09-18); a frame of `nan` and `inf` has an answer in it and does not warn.
...
- **Provenance boundary (C-39):** `TestSourceGitShaBoundaryRed` (wheel inside a consumer's checkout, vendored copy under a consumer's root, unreadable `.git` logs and stamps bare), `TestSourceGitShaBoundaryGreen` (loose ref, packed ref, PEP 621 hyphenated name, linked worktree on a branch via `commondir`), `TestSourceGitShaBoundaryBeige` (submodule-style relative `gitdir:` resolves against the `.git` file).
...
- **`scoring_code_version` identifies the code that ran, when it can.** It is the installed distribution's version, plus `+g<sha>` when the package is running from **this repository's own checkout** (e.g. `1.0.0+g5469690`). The SHA is read from one place only: a `.git` directory, or `.git`-file pointer, in the directory containing the `views_evaluation/` package, and only when a `pyproject.toml` there declares this distribution. A wheel install therefore stamps a bare version wherever it lives, including inside another repository's checkout (register C-39) — a property of the install, not a failure. A `.git` that is ours but yields no commit (unreadable, corrupt, symbolic-ref chain, unborn branch) also stamps bare, and logs at WARNING. A wheel built from an untagged commit stamps the version it was built as, indistinguishable from the tagged release. The SHA exists because `importlib.metadata` reports the *installed distribution*, not the executing code: under an editable install those drift as soon as the source moves ahead of the last `pip install`, and a bare version cannot distinguish the two. Register **C-25**.
