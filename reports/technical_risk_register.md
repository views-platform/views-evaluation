...
> **2026-09-17 — epic #66 (pandas-free), story S1 (#67) closed C-39.** `_source_git_sha` no longer
> walks parent directories; it reads `.git` only at the package's own root and only when a
> `pyproject.toml` there names `views_evaluation`, so a wheel inside a consumer's checkout
> stamps a bare version. Three constructed-tree tests cover the consumer-venv, own-checkout
> and relative-`gitdir:` cases; the consumer-venv and relative-`gitdir:` cases were observed
...
> **2026-09-19 — release 2.1.0.** Carries #91 only. First release to answer ADR-022 §7's new sixth
> checklist item (stale caps): views-frames fixed, numpy and scipy caps noted as measured (C-41).
> §3.2 notification to views-reporting before the tag (#289, comment 5738670537), stating that their
> own direct views-frames bound still excludes 2.x. The release commit's guard audit (63 mutations)
> found the specific text guards weak behind the held-text backstop: nine exit-status evasions the
> suppressed list did not name (all proven to exit 0 under `bash -e`), a specifier reader that took the
> first of several declarations, and floor-step checks satisfied by tokens in the wrong place. All
> closed — the list carries the nine forms, the reader requires exactly one canonical declaration, the
> floor guard is structural (derivation → install → check → proof → pytest, FLOOR assigned once) — and
> re-verified red. Count unchanged.
...
> floor-step checks that a `-k` or `|| true` slipped past), all closed and re-verified red. The release review then found the floor-step guard's `continue-on-error` check vacuous (it read a key the parser never emits) and its derivation check token-matchable; both replaced — the step now calls the one specifier reader and the guard asserts the call, the parser's own flag, the full suppressed-exit list, `--no-deps`, `pip check` and an import proof on the floor. Ships as
...
> **2026-09-18 — decision D-01 recorded** (a new *Decisions* section): `AP` on a group with no
> positive truth returns `nan` with the warning suppressed, per ADR-015 R9, ruled by the maintainer
> ahead of the AP kernel switch in #64. Count unchanged.
...
> workflow to a known-good text, and a CHANGELOG guard fails when the declared version has
> no released section. C-36 stays open on its notification, branch-bypass and
> checklist-content halves. Count unchanged.
...
- **Note:** ADR-015's Validation & Monitoring section gained a standing rule from this — *execute the real caller's path, not a reconstruction of it*. Related to C-33 (the config half) and C-24 (the emit half). Both reversals in ADR-015 (rulings 2 and 7) were found by running a real workflow, never by the suite. Third instance (2026-09-16): C-39 was found by simulating an install layout — a wheel inside a consumer's `.venv` — that this repo's CI never runs.
...
- **Trigger:** When a downstream consumer aggregates a sentinel-bearing metric — `Pearson`, `MCR`, and since 2026-09-18 `AP` (ADR-015 R9) — across groups without excluding `nan`, a single degenerate group silently poisons the aggregate to `nan`. `to_metric_frame()` uses `nanmean` and is safe. **The consumer that reads `to_dict()` directly is views-pipeline-core's WandB logging** (`modules/wandb/utils.py`, a mean skipping `None` only; observed 2026-09-18), which is live for `AP` via `views-models/ensembles/rusty_bucket` — see D-01.
...
- **Tier:** 2 (High)
- **Description:** ADR-022 §7 names a **release checklist** (five items until 2026-09-19, six since — the sixth asks about stale dependency caps, #91) as the policy's enforcement point, "worked through before any `poetry publish`". Nothing requires it to be worked through. It is a markdown list inside an ADR: no CI job, no template, no artifact that records it happened. The same is true of §3.2, which requires that "downstream consumers known to be affected must be **notified before** the release is cut, not after" — there is no place a notification is recorded, so its absence is indistinguishable from its presence. On the very first release governed by this policy (0.5.0, PR #45), the code review found the checklist had not been worked through anywhere, no `CHANGELOG.md` existed at all, and no committed artifact evidenced consumer notification — despite ADR-022 §6 stating outright that "the 0.5.0 release notes **must** retrospectively record the 0.4.0 removals as breaking".
...
- **Trigger:** (a) The next release that changes behaviour for a known consumer without that consumer being told first (§3.2) — nothing records that a notification happened. (b) A release cut from a branch no pull request ever ran on: the guards run inside the suite they gate, so a neutered `publish_package.yml` on such a branch is not caught. (c) A `## [version]` section that exists but whose checklist was not worked through — the guard sees the heading, a real date and non-empty content, not the checklist.
- **Location:** `documentation/ADRs/022_evolution_and_stability.md` §3.2 (notification) and §7 (the checklist); `.github/workflows/publish_package.yml`; `tests/test_falsification_extras_actually_installed.py` (`TestPublishGateIsReal`); `tests/test_documentation_contracts.py` (`TestChangelogCoversTheDeclaredVersion`); `CHANGELOG.md`
- **Mechanical half (added 2026-09-16, repo-assimilation; closed 2026-09-17).** Until #68, the only automated gate between a GitHub release event and PyPI was "the version is higher than the last one on PyPI": `publish_package.yml` ran no tests and did not depend on `run_pytest.yml`, so a release cut from a red commit would have published. Independently checkable and independently fixable, separate from the checklist and notification halves.
...
- **Note:** Cross-refs — C-34 (whose mitigation this undermines) and C-35 (both are instances of verifying against an artifact chosen by the verifier rather than against reality). Further instance: C-39's guard (`test_scoring_code_version_identifies_the_code_that_ran`) was green because it could only see this repo's own `.git`; closed by #67 with constructed-tree fixtures.
...
- **Location:** `pyproject.toml` (dependency constraints); `views-reporting/pyproject.toml` (formerly the `[tool.uv.sources]` branch-tracking entry; a plain version range since 2026-09-16 — its value is theirs and moves; read it there, not here)
...
- **Trigger fired, unnoticed (recorded 2026-09-19):** scipy 1.18.0 (2026-06-19) requires `numpy>=2.0.0` and `python>=3.12`. Under our numpy cap the resolver did not go red — it backtracked to scipy 1.17.x silently (CI run 35416248120 installed scipy 1.17.1 and printed nothing about it), which is the outcome neither clause of the description foresaw: not an unsatisfiable resolve but a frozen effective scipy ceiling. Found by the 2.1.0 release review, three months late. The numpy cap is therefore **stale, not deliberate**: it has never been measured under numpy 2 and it excludes two declared interpreters. Tracked as **#93** (measure numpy 2.x, lift or narrow `python`).
- **Location:** `pyproject.toml` (`numpy = "^1.26.4"`, `python = ">=3.11,<3.15"`, `scipy = "^1.11"` whose effective ceiling is now set by the numpy cap, `views-frames >=1.10.2,<3`); `.github/workflows/run_pytest.yml` (`python-version: "3.11"`; `poetry install --all-extras` without a lock); absence of `poetry.lock`
...
- **Note:** Inward-facing counterpart of C-38 — that entry is about consumers' resolution being unchecked; this is about this repo's own resolution being unrecorded. Same evidence gap as C-35: "it passed" without "against what". **2026-09-18:** the resolve shrank — scikit-learn left the runtime dependencies (C-05, #64) and is now dev-only; the parity suite's oracle is whatever scikit-learn CI resolves, which this entry's lockfile point now also covers.
...
Maintainer decisions taken on a register concern, recorded here so the reasoning outlives the session that produced it (ADR-023). A decision is not a concern: it has no tier and no trigger, and it closes when its consequence ships.

### D-01 — `AP` on a group with no positive truth: `nan` sentinel, warning suppressed
- **Decided:** 2026-09-18, by the maintainer, on the three options laid out with the code in front of them. Recorded in full as ADR-015 ruling 9 (R9); this entry is the pointer and the consequence tracker.
...
### Closed by the pandas-free epic (#66), 2026-09-17
...
| ID | Tier | Description | Resolution | Issue |
|----|------|-------------|------------|-------|
| C-25 | 3 | Provenance version-label integrity in the evaluation-of-record | `default_scoring_code_version()` now appends `+g<sha>` when the package runs from a git worktree, so the stamp identifies the executing code rather than the last `pip install`. A wheel install has no worktree and stamps a bare version — a property of the install, not a failure (ADR-015). *(Wording superseded 2026-09-17: a wheel can sit inside a consumer's worktree, and this fix then stamped the consumer's SHA — see C-39, closed by #67.)* The entry's original proposal, "cut the 0.5.0 release so the label is meaningful", was falsified: releasing does not help, because the drift is between the installed distribution and the source tree, not between versions. Demonstrated with the tree at 1.0.0 and the dist-info at 0.5.0 emitting frames stamped `0.5.0`. The guarding test previously asserted only that the stamp was a non-empty string, so it was satisfied by exactly the wrong answer; it now asserts the stamp identifies HEAD. | #57 |
